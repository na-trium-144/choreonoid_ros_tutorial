...
public:
  ...
  virtual bool initialize(SimpleControllerIO *io) override {
    ...
    subscriber = node->subscribe(
        "cmd_vel", 1, &RttTankController::command_velocity_callback, this);
    return true;
  }
  ...
  virtual bool control() override {
    ...
    trackL->dq_target() =
        0.5 * command_velocity.linear.x - 0.3 * command_velocity.angular.z;
    trackR->dq_target() =
        0.5 * command_velocity.linear.x + 0.3 * command_velocity.angular.z;
  ...
  virtual void stop() override { subscriber.shutdown(); }
};
